    ...
    room.mirrors.forEach(mirror => {
        ...
        for (let angle = 0; angle < 360; angle += 5) {
            ...
            room.cipherSymbols.forEach(symbol => {
                ...
                if (dist < 30) {
                    validHits.push({ target: symbol.symbol, angle: currentRotation, dist: dist });
                }
            });
        }
        ...
        let bestHits = {};
        validHits.forEach(h => {
            if (!bestHits[h.target] || bestHits[h.target].dist > h.dist) {
                bestHits[h.target] = h;
            }
        });
        
        for (let target in bestHits) {
            console.log(`Mirror ${mirror.id} hits '${target}' at rotation ${bestHits[target].angle}° (dist: ${bestHits[target].dist.toFixed(2)})`);
        }
    });
});
