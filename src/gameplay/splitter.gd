...
## Returns straight and right-angled split beams, or empty array if blocked.
func process_beam(incoming_origin: Vector2, incoming_dir: Vector2, incoming_color: Color) -> Array[Dictionary]:
	...
	# Validate front-face hit: dot product must be <= -0.5
	if incoming_dir.dot(global_normal) > -0.5:
		return []
		
	var straight_dir := incoming_dir.snapped(Vector2(0.001, 0.001)).normalized()
	
	# Local right is local Vector2.RIGHT (snapped to precision)
	var right_dir := Vector2.RIGHT.rotated(rotation).snapped(Vector2(0.001, 0.001)).normalized()
...
## Helper method for tests to snap/rotate the prism's Y rotation
func rotate_prism_by(angle_radians: float) -> void:
